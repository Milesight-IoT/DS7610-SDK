...
| supportsJoin   | R/W bool   | true:OTAA false:ABP |
| supportsClassB | R/W bool   | 是否支持CLASS B     |
| supportsClassC | R/W bool   | 是否支持CLASS C     |
...
| devEUI       | R/W string(HEX16) | 仅创建时可写，创建后禁止修改           |
| profileName  | R/W string        | 模板名称                               |
| appKey       | R/W string(HEX32) | 仅 OTAA 时可写                         |
| devAddr      | R/W string(HEX8)  | 仅 ABP 时可写                          |
| appSKey      | R/W string(HEX32) | 仅 ABP 时可写                          |
| nwkSKey      | R/W string(HEX32) | 仅 ABP 时可写                          |
| fCntUp       | R/W int/string    | 仅 ABP 时可写，创建为0，修改时保留原值 |
| fCntDown     | R/W int/string    | 仅 ABP 时可写，创建为0，修改时保留原值 |
| active       | R bool            | 入网状态                               |
| supportsJoin | R bool            | otaa:true abp:false 根据查询的值设置   |
...
| 接口参数     | SDK选项           | 备注                                   |
| ------------ | ----------------- | -------------------------------------- |
| devicename   | R/W string        |                                        |
| type         | R string          | joinreq accept upunc upcnf dnunc dncnf |
| payloadHex   | R string(HEX)     |                                        |
| fPort        | R int/string      |                                        |
| fCnt         | R int/string      |                                        |
| time         | R string          |                                        |
| rssi/loraSNR | R string(int/int) |                                        |
